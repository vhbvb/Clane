...
namespace
{
    ...
    class MobPluginVisitor : public RecursiveASTVisitor<MobPluginVisitor>
    {
    private:
    ...
    public:
        ...
        MobPluginVisitor (CompilerInstance &Instance)
        :Instance(Instance)
        {
...
//                    D.Report(loc.getLocWithOffset(lOffset), diagID).AddFixItHint(fixItHint); //类扩展检测协议格式xcode会崩 >..<
                    D.Report(loc.getLocWithOffset(lOffset), diagID);
                }
            }
        };
        ...
        void checkThreadOperationInInitialize (ObjCMethodDecl *decl)
        {
            ...
            for (int i = 0; i < selectorPartCount; i++)
            {
                ...
                if (decl -> hasBody())
                {
                    ...
                    {
                        checkMethodBody(methodBody->getLocStart(), srcCode.c_str(), srcCode.c_str(), "dispatch_sync", "Avoid thread operation in class method \"initialize\"");
                        checkMethodBody(methodBody->getLocStart(), srcCode.c_str(), srcCode.c_str(), "dispatch_async", "Avoid thread operation in class method \"initialize\"");
                    }
                    
                    checkMethodBody(methodBody->getLocStart(), srcCode.c_str(), srcCode.c_str(), "[NSNumber alloc]", "You should use literal quantities to define it !");
                    checkMethodBody(methodBody->getLocStart(), srcCode.c_str(), srcCode.c_str(), "NSNumber numberWith", "You should use literal quantities to define it !");
                    checkMethodBody(methodBody->getLocStart(), srcCode.c_str(), srcCode.c_str(), "NSDictionary dictionaryWithObjectsAndKeys:", "You should use literal quantities to define it !");
                    checkMethodBody(methodBody->getLocStart(), srcCode.c_str(), srcCode.c_str(), "[NSDictionary alloc]", "You should use literal quantities to define it !");
                    checkMethodBody(methodBody->getLocStart(), srcCode.c_str(), srcCode.c_str(), "NSArray arrayWithObjects:", "You should use literal quantities to define it !");
                    checkMethodBody(methodBody->getLocStart(), srcCode.c_str(), srcCode.c_str(), "[NSArray alloc]", "You should use literal quantities to define it !");
                }
            }
        }
        
        
        bool checkMethodBody(SourceLocation startLoc, const char *sourceCode, const char *targetBody, const char *target, std::string hintMsg)
        {
            ...
            if (targetFuntion != NULL)
            {
                ...
                checkMethodBody(startLoc, sourceCode, targetFuntion + strlen(target), target, hintMsg);
            }
    ...
    class MobPluginConsumer : public ASTConsumer
    {
        CompilerInstance &Instance;
        std::set<std::string> ParsedTemplates;
    public:
        MobPluginConsumer(CompilerInstance &Instance,
                          std::set<std::string> ParsedTemplates)
    ...
    private:
        MobPluginVisitor visitor;
    };
    
    
    
    class MobPluginASTAction : public PluginASTAction
    {
        std::set<std::string> ParsedTemplates;
    protected:
        ...
        {
            return llvm::make_unique<MobPluginConsumer>(CI, ParsedTemplates);
        }
...
static clang::FrontendPluginRegistry::Add<MobPluginASTAction>
X("MobPlugin", "MobPlugin");
